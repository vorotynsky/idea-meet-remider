...
class SpaceLoginAction : AnAction("Login to Space for Meet Reminder") {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        CalendarStorage.spaceCalendarProvider.login(URL(url), password) {
            if(it == null) Notification("Plugin Error", "Invalid login or password.", NotificationType.ERROR).notify(e.project)
            else Notification("Print", "Login success!", NotificationType.INFORMATION).notify(e.project)
        }
